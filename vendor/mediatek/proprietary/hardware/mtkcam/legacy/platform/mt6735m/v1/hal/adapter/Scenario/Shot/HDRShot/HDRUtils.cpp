...
updateInfo()
{
    ...
    if(NSCam::SENSOR_TYPE_YUV == mSensorType){
        MY_DBG("[init] - mSensorType SENSOR_TYPE_YUV");
    } else if(NSCam::SENSOR_TYPE_RAW == mSensorType){
        MY_DBG("[init] - mSensorType SENSOR_TYPE_RAW");
    } else {
        MY_ERR("[init] - mSensorType unknown:%d",mSensorType);
    }
    ...
    #else
    // in round 2 case, we send a small hdr frame for user
    // preview the result. but it extended shot to shot time.
    // now, default disable it, reopen if working sensor
    // is larger then 8 MP .
    mHdrRoundTotal = 1;
    #endif
...
requestSourceImgBuf(void)
{
    ...
    for (MUINT32 i = 0; i < u4OutputFrameNum; i++)
    {
        ...
        MY_VERB("[requestBufs] mpSourceImgBuf[%d].virtAddr: %zu.", i, mpSourceImgBuf[i].virtAddr);
        MY_VERB("[requestBufs] mpSourceImgBuf[%d].phyAddr : %zu.", i, mpSourceImgBuf[i].phyAddr);
...
requestFirstRunSourceImgBuf(void)
{
    ...
    for (MUINT32 i = 0; i < u4OutputFrameNum; i++)
    {
        ...
        MY_VERB("[requestBufs] mpFirstRunSourceImgBuf[%d].virtAddr: %zu.", i, mpFirstRunSourceImgBuf[i].virtAddr);
        MY_VERB("[requestBufs] mpFirstRunSourceImgBuf[%d].phyAddr : %zu.", i, mpFirstRunSourceImgBuf[i].phyAddr);
...
requestSmallImgBuf(void)
{
    ...
    for (MUINT32 i = 0; i < mu4OutputFrameNum; i++)
    {
        ...
        MY_VERB("[requestSmallImgBuf] mpSmallImgBuf[%d].virtAddr: %zu.",    i, mpSmallImgBuf[i].virtAddr);
        MY_VERB("[requestSmallImgBuf] mpSmallImgBuf[%d].phyAddr : %zu.",    i, mpSmallImgBuf[i].phyAddr);
...
requestSEImgBuf(void)
{
    ...
    for (MUINT32 i = 0; i < mu4OutputFrameNum; i++)
    {
        ...
        MY_VERB("[requestSEImgBuf] mpSEImgBuf[%d].virtAddr: 0x%zu.",    i, mpSEImgBuf[i].virtAddr);
        MY_VERB("[requestSEImgBuf] mpSEImgBuf[%d].phyAddr : 0x%zu.",    i, mpSEImgBuf[i].phyAddr);
...
requestNormalJpegBuf(void)
{
    ...
    MY_DBG("[requestNormalJpegBuf] mNormalJpegBuf.virtAddr: %zu.",    mNormalJpegBuf.virtAddr);
    MY_DBG("[requestNormalJpegBuf] mNormalJpegBuf.phyAddr : %zu.",    mNormalJpegBuf.phyAddr);
...
requestNormalThumbnailJpegBuf(void)
{
    ...
    MY_DBG("[requestNormalThumbnailJpegBuf] mNormalThumbnailJpegBuf.virtAddr: %zu.",    mNormalThumbnailJpegBuf.virtAddr);
    MY_DBG("[requestNormalThumbnailJpegBuf] mNormalThumbnailJpegBuf.phyAddr : %zu.",    mNormalThumbnailJpegBuf.phyAddr);
...
requestHdrJpegBuf(void)
{
    ...
    MY_DBG("[requestHdrJpegBuf] mHdrJpegBuf.virtAddr: %zu.",    mHdrJpegBuf.virtAddr);
    MY_DBG("[requestHdrJpegBuf] mHdrJpegBuf.phyAddr : %zu.",    mHdrJpegBuf.phyAddr);
...
requestHdrThumbnailJpegBuf(void)
{
    ...
    MY_DBG("[requestHdrThumbnailJpegBuf] mHdrThumbnailJpegBuf.virtAddr: 0x%zu.",    mHdrThumbnailJpegBuf.virtAddr);
    MY_DBG("[requestHdrThumbnailJpegBuf] mHdrThumbnailJpegBuf.phyAddr : 0x%zu.",    mHdrThumbnailJpegBuf.phyAddr);
...
requestHdrWorkingBuf(void)
{
    ...
    MY_DBG("[requestHdrWorkingBuf] mpHdrWorkingBuf.virtAddr: %zu.",    mpHdrWorkingBuf.virtAddr);
    MY_DBG("[requestHdrWorkingBuf] mpHdrWorkingBuf.phyAddr : %zu.",    mpHdrWorkingBuf.phyAddr);
...
requestOriWeightMapBuf(void)
{
    ...
    for (MUINT32 i = 0; i < mu4OutputFrameNum; i++)
    {
        ...
        MY_DBG("[requestOriWeightMapBuf] addr[%d] = %zu", i, mWeightingBuf[i].virtAddr);
    }
...
requestDownSizedWeightMapBuf(void)
{
    ...
    for (MUINT32 i = 0; i < mu4OutputFrameNum; i++)
    {
        ...
        MY_VERB("[requestDownSizedWeightMapBuf] mpDownSizedWeightMapBuf[%d].virtAddr: %zu.",    i, mpDownSizedWeightMapBuf[i].virtAddr);
        MY_VERB("[requestDownSizedWeightMapBuf] mpDownSizedWeightMapBuf[%d].phyAddr : %zu.",    i, mpDownSizedWeightMapBuf[i].phyAddr);
...
requestPostviewImgBuf(void)
{
    ...
    MY_VERB("[requestPostviewImgBuf] mpPostviewImgBuf.virtAddr: %zu.",    mpPostviewImgBuf.virtAddr);
    MY_VERB("[requestPostviewImgBuf] mpPostviewImgBuf.phyAddr : %zu.",    mpPostviewImgBuf.phyAddr);
...
handleJpegData(MUINT8* const puJpegBuf, MUINT32 const u4JpegSize, MUINT8* const puThumbBuf, MUINT32 const u4ThumbSize, MUINT32 const u4Index, MBOOL bFinal)
{
    ...
    {
        char szFileName[100];
        if(NULL != puJpegBuf && 0!= u4JpegSize){
            ::sprintf(szFileName, HDR_DEBUG_OUTPUT_FOLDER "%04d_10_mHdrJpegBuf_%dx%d.jpg", mu4RunningNumber, mrHdrCroppedResult.output_image_width, mrHdrCroppedResult.output_image_height);
            dumpToFile(szFileName, (MUINT8*)puJpegBuf, u4JpegSize);
        }
    }
    // Jpeg callback
...
createSourceAndSmallImg(void)
{
    ...
    MY_DBG("lock AWB");
    p3AHal->setAWBLock(MTRUE);

    MY_DBG("Disable TSF E3ACtrl_SetShadingTSFCapByp");
    ret = ret && !p3AHal->send3ACtrl(E3ACtrl_SetShadingTSFCapByp,1,0);
...
#if !HDR_DEBUG_SKIP_3A
    MY_DBG("unlock AWB");
    p3AHal->setAWBLock(MFALSE);

    MY_DBG("Enable TSF E3ACtrl_SetShadingTSFCapByp");
    ret = ret && !p3AHal->send3ACtrl(E3ACtrl_SetShadingTSFCapByp,0,0);
...
createNormalJpegImgTask(MVOID* arg)
{
    ...
    if(!self->mfgIsForceBreak) {
        ret = ret && (MUINTPTR)self->encodeNormalThumbnailJpeg(arg);
    }
    if(!self->mfgIsForceBreak) {
        ret = ret && (MUINTPTR)self->encodeNormalJpeg(arg);
    }
    if(!self->mfgIsForceBreak) {
        ret = ret && (MUINTPTR)self->saveNormalJpeg(arg);
    }
...
MVOID*
HdrShot::
encodeNormalThumbnailJpeg(MVOID *arg __attribute__((unused)))
{
    HDR_TRACE_CALL();
    FUNCTION_LOG_START;
    MUINTPTR     ret = MTRUE;
...
MVOID*
HdrShot::
encodeNormalJpeg(MVOID *arg __attribute__((unused)))
{
    HDR_TRACE_CALL();
    FUNCTION_LOG_START;
    MUINTPTR     ret = MTRUE;
...
MVOID*
HdrShot::
saveNormalJpeg(MVOID *arg __attribute__((unused)))
{
    HDR_TRACE_CALL();
    FUNCTION_LOG_START;
    MUINTPTR     ret = MTRUE;
...
MVOID*
HdrShot::
encodeHdrThumbnailJpeg(MVOID *arg __attribute__((unused)))
{
    HDR_TRACE_CALL();
    FUNCTION_LOG_START;
    MUINTPTR     ret = MTRUE;
...
MVOID*
HdrShot::
encodeHdrJpeg(MVOID *arg __attribute__((unused)))
{
    HDR_TRACE_CALL();
    FUNCTION_LOG_START;
    MUINTPTR     ret = MTRUE;
...
MVOID*
HdrShot::
saveHdrJpeg(MVOID *arg __attribute__((unused)))
{
    HDR_TRACE_CALL();
    FUNCTION_LOG_START;
    MUINTPTR     ret = MTRUE;
    ...
    {
        char szFileName[100];
        if(0 != mHdrThumbnailJpegBuf.virtAddr && 0!= mHdrThumbnailJpegBufSize){
            ::sprintf(szFileName, HDR_DEBUG_OUTPUT_FOLDER "%04d_11_mHdrThumbnailJpegBuf_%dx%d.jpg", mu4RunningNumber, mJpegParam.mi4JpegThumbWidth , mJpegParam.mi4JpegThumbHeight);
            dumpToFile(szFileName, (MUINT8*)mHdrThumbnailJpegBuf.virtAddr, mHdrThumbnailJpegBufSize);
        }
    }
...
MINT32
HdrShot::
mHalCamHdrProc(HdrState_e eHdrState __attribute__((unused)))
{
    return 0;
}
...
updateThumbnailExif(CamExif *pCamExif, MUINT8* const puThumbBuf, MUINT32 const u4ThumbSize, MUINT8* puExifBuf, MUINT32 &u4FinalExifSize) {
    ...
    MY_DBG("pCamExif=%p, puThumbBuf=%p, u4ThumbSize=%d, puExifBuf=%p, u4FinalExifSize=%d",
...
querySensorInfo(NSCam::SensorStaticInfo &mSensorInfo)
{
    ...
    {
        MY_ERR("wrong sensor idx(0x%x), sensorNum(%d)", getOpenId(), sensorNum);
        goto lbExit;
    }
...
getCaptureParams(NS3A::CaptureParam_T *pCap3AParam)
{
    ...
    for(MUINT32 i=0; i<strHDROutputSetting.u4OutputFrameNum; i++) {
        MY_DBG("[getCaptureParams] HDR Exposuretime[%d] : %d Gain[%d]:%d Flare[%d]:%d\n"
    ...
    CHECK_OBJECT(p3AHal);
    for(MUINT32 i=0; i<strHDROutputSetting.u4OutputFrameNum; i++) {
        p3AHal->getCaptureParams(i, 0, pCap3AParam[i]);
    }
    p3AHal->destroyInstance();
#endif
    ...
    for(MUINT32 i=0; i<strHDROutputSetting.u4OutputFrameNum; i++) {
        ...
        MY_DBG("[getCaptureParams] Modify Exposuretime[%d] : %d AfeGain[%d]:%d IspGain[%d]:%d ISO[%d]:%d\n"
    ...
    #if 0 //TODO:  D2 aaa_hal_common.h didn' t support i4YuvEvIdx?
    if(mSensorType == NSCam::SENSOR_TYPE_YUV)
    {
        // for yuv sensor - take 2 pictures, -1.0 & 1.5 ev
        pCap3AParam[0].i4YuvEvIdx = LIB3A_AE_EV_COMP_n10;
        pCap3AParam[1].i4YuvEvIdx = LIB3A_AE_EV_COMP_15;
        MY_DBG("[getCaptureParams] Modify i4YuvEvIdx [%d, %d]"
...
getCaptureExposureSettings(HDRExpSettingOutputParam_T &strHDROutputSetting)
{
    ...
    if(mSensorType == NSCam::SENSOR_TYPE_YUV) {
        ...
        #if 1
        if(sizeof(HDRExpSettingInputParam_T)!=sizeof(ExpSettingParam_T)){
            MY_ERR("HDRExpSettingInputParam_T != ExpSettingParam_T, fix it");
        }
        if(sizeof(HDRExpSettingInputParam_T::u4Histogram)!=sizeof(ExpSettingParam_T::u4Histogram)){
            MY_ERR("HDRExpSettingInputParam_T::u4Histogram != ExpSettingParam_T::u4Histogram, fix it");
        }
        #else
