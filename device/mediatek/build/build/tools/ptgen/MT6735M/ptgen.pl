...
use lib "$LOCAL_PATH/../utils/";
require 'ptgen_utils.pl';
use Data::Dumper;
*Dump = \&Dumper;
...
{
    @partition_layout_raw = @{&ReadCSVFile_Common($Partition_layout_xls)};
    &debug_print_layout("$ENV{OUT_DIR}/1.log", \@partition_layout_raw);
}
else
{
    @partition_layout_raw = @{&ReadPlainText($Partition_layout_yaml)};
    &debug_print_layout("$ENV{OUT_DIR}/2.log", \@partition_layout_raw);
}
@partition_layout_cooked = @{&ProcessRawPartitionLayoutData(\@partition_layout_raw)};
print "###################Final Data###################\n";
print Dump(\@partition_layout_cooked);
&GenYAMLScatFile();
if ($ArgList{EMMC_SUPPORT} eq "yes" || $ArgList{UFS_BOOTING} eq "yes" || $ArgList{NAND_UBIFS_SUPPORT} eq "yes")
{
    &GenPartSizeFile_iniFile($PART_SIZE_LOCATION, $PRODUCT_OUT);
}
if ($ArgList{EMMC_SUPPORT} eq "yes" || $ArgList{UFS_BOOTING} eq "yes")
{
   &GenPGPTFile();
}
if ($ArgList{EMMC_SUPPORT} ne "yes" && $ArgList{UFS_BOOTING} ne "yes")
{
    &GenLK_PartitionC();
...
=cut
    ...
    $ArgList{UFS_OTP}                    = $ENV{MTK_UFS_OTP};
    $ArgList{UFS_BOOTING}                = $ENV{MTK_UFS_SUPPORT};
    ...
    $ArgList{TINYSYS_SSPM_SUPPORT} = $ENV{MTK_TINYSYS_SSPM_SUPPORT};
    $ArgList{SIM_LOCK_POWER_ON_WRITE_PROTECT} = $ENV{MTK_SIM_LOCK_POWER_ON_WRITE_PROTECT};
    $ArgList{VPU_SUPPORT} = $ENV{MTK_VPU_SUPPORT};
    $ArgList{SPM_FW_USE_PARTITION} = $ENV{SPM_FW_USE_PARTITION};
    $ArgList{MCUPM_FW_USE_PARTITION} = $ENV{MCUPM_FW_USE_PARTITION};
    $ArgList{MTK_A1_FEATURE}             = $ENV{MTK_A1_FEATURE};
    $ArgList{MTK_TC1_FEATURE}            = $ENV{MTK_TC1_FEATURE};
    $ArgList{MTK_NVCONFIG_PARTITION_SUPPORT}            = $ENV{MTK_NVCONFIG_PARTITION_SUPPORT};
    $ArgList{MTK_DRAMC_BOOT_OPT}            = $ENV{MTK_DRAMC_BOOT_OPT};
    $ArgList{DTBO_FEATURE}               = $ENV{MTK_DTBO_FEATURE};
    $ArgList{MTK_AB_OTA_UPDATER}         = $ENV{MTK_AB_OTA_UPDATER};
    $ArgList{MTK_RESERVE_ENG_PARTITIONS} = $ENV{MTK_RESERVE_ENG_PARTITIONS};
    $ArgList{TARGET_COPY_OUT_ODM}                = $ENV{TARGET_COPY_OUT_ODM};
    $ArgList{MTK_SINGLE_BIN_MODEM_SUPPORT} = $ENV{MTK_SINGLE_BIN_MODEM_SUPPORT};
    $ArgList{MAIN_VBMETA_IN_BOOT}          = $ENV{MAIN_VBMETA_IN_BOOT};
    $ArgList{MTK_LOADER_BACKUP}          = $ENV{MTK_LOADER_BACKUP};
    $ArgList{MTK_COMMON}                 = "common";
    $ArgList{MTK_GMO_RAM_OPTIMIZE}       = $ENV{MTK_GMO_RAM_OPTIMIZE};
    $ArgList{MTK_BOARD_AVB_ENABLE}         = $ENV{MTK_BOARD_AVB_ENABLE};
    $ArgList{DTBO_UPGRADE_FROM_ANDROID_O}               = $ENV{MTK_DTBO_UPGRADE_FROM_ANDROID_O};
    $ArgList{MTK_BOARD_USES_METADATA_PARTITION}         = $ENV{MTK_BOARD_USES_METADATA_PARTITION};

    ##for FPGA SD bootup(MTK_EMMC_SUPPORT=no and UFS_BOOTING=no) build pass
    if ($ENV{BUILD_MTK_LDVT} eq "yes" && $ArgList{EMMC_SUPPORT} ne "yes" && $ArgList{UFS_BOOTING} ne "yes")
    {
    ...
    if ($ENV{PRODUCT_USE_DYNAMIC_PARTITIONS} eq "true")
    {
        $ArgList{DYNAMIC_PARTITION_SUPPORT} = "yes";
    }
    if ($ENV{PRODUCT_RETROFIT_DYNAMIC_PARTITIONS} eq "true")
    {
        $ArgList{PRODUCT_RETROFIT_DYNAMIC_PARTITIONS} = "yes";
    }
    if ($ENV{TARGET_NO_RECOVERY} eq "true")
    {
        $ArgList{TARGET_NO_RECOVERY} = "yes";
    }
    ...
    if ($ArgList{PL_MODE})
    {
        $Partition_layout_xls = "$ptgen_location/test_partition_table_internal_$ArgList{PLATFORM}";
        if ($ArgList{UFS_BOOTING} eq "yes")
        {
            $ArgList{SHEET_NAME} = "$ArgList{PL_MODE}_UFS";
        }
        elsif ($ArgList{EMMC_SUPPORT} eq "yes")
        {
            $ArgList{SHEET_NAME} = "$ArgList{PL_MODE}_EMMC";
        }
    }
    else
    {
    ...
    $Partition_layout_xls = "${Partition_layout_xls}_$ArgList{SHEET_NAME}.csv";
    ...
    if (($ArgList{MTK_AB_OTA_UPDATER} eq "yes") && ($ArgList{MTK_RESERVE_ENG_PARTITIONS} ne "yes"))
    {
        %AlignPartList = (
                      persist => 8388608,
                      sec1 => 8388608,
...
sub ProcessRawPartitionLayoutData
{
    ...
    &debug_print_layout("$ENV{OUT_DIR}/3.log", \@partition_layout_process);
    ...
    if ($board_config)
    {
        ...
        while (defined($line = <$board_config>))
        {
            ...
            if ($line =~ /\A\s*TARGET_COPY_OUT_PRODUCT\s*:=\s*(.*)/i)
            {
                if ($1 eq "product")
                {
                    $product_partition_exists = "Y";
                    print "by common enable product partition\n";
                }
                else
                {
                    $product_partition_exists = "N";
                }
            }
    ...
    if ($board_config)
    {
        ...
        while (defined($line = <$board_config>))
        {
            ...
            if ($line =~ /\A\s*TARGET_COPY_OUT_PRODUCT\s*:=\s*(.*)/i)
            {
                ...
                {
                    $product_partition_exists = "Y";
                    print "by platform enable product partition\n";
                }
                else
                {
                    $product_partition_exists = "N";
                    print "by platform disable product partition\n";
                }
            }
            foreach my $part (@partition_layout_process)
            {
    ...
    if ($board_config)
    {
        ...
        while (defined($line = <$board_config>))
        {
            ...
            if ($line =~ /\A\s*TARGET_COPY_OUT_PRODUCT\s*:=\s*(.*)/i)
            {
                ...
                {
                    $product_partition_exists = "Y";
                    print "by project enable product partition\n";
                }
                else
                {
                    $product_partition_exists = "N";
                    print "by project disable product partition\n";
                }
            }
            foreach my $part (@partition_layout_process)
            {
        ...
        if ($flavor_board_config)
        {
            ...
            while (defined($line = <$flavor_board_config>))
            {
                ...
                if ($line =~ /\A\s*TARGET_COPY_OUT_PRODUCT\s*:=\s*(.*)/i)
                {
                    ...
                    {
                        $product_partition_exists = "Y";
                        print "by flavor project enable product partition\n";
                    }
                    else
                    {
                        $product_partition_exists = "N";
                        print "by flavor project disable product partition\n";
                    }
                }
    ...
    &debug_print_layout("$ENV{OUT_DIR}/5.log", \@partition_layout_process);
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        ...
        {
            if ((($ArgList{EMMC_SUPPORT} eq "yes" || $ArgList{UFS_BOOTING} eq "yes") && $ArgList{SHARED_SDCARD} eq "yes") || ($ArgList{EMMC_SUPPORT} ne "yes" && $ArgList{UFS_BOOTING} ne "yes" && $ArgList{FAT_ON_NAND} ne "yes"))
            {
        ...
        {
            if (($ArgList{EMMC_SUPPORT} eq "yes" && $ArgList{EMMC_SUPPORT_OTP} ne "yes") || ($ArgList{UFS_BOOTING} eq "yes" && $ArgList{UFS_OTP} ne "yes"))
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "recovery2")
        {
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "nvcfg")
        {
            if ($ArgList{MTK_NVCONFIG_PARTITION_SUPPORT} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "lk2")
        {
            if ($ArgList{MTK_LOADER_BACKUP} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        ...
        {
            if ($ArgList{MTK_DRAMC_BOOT_OPT} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^spmfw(_[ab])?$/i)
        {
            if ($ArgList{SPM_FW_USE_PARTITION} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^mcupmfw(_[ab])?$/i)
        {
            if ($ArgList{MCUPM_FW_USE_PARTITION} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^sspm_([12]|[ab])$/i)
        {
            if ($ArgList{TINYSYS_SSPM_SUPPORT} ne "yes")
            {
               splice @partition_layout_process, $partition_idx, 1;
               $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^cam_vpu[1-3](_[ab])?$/i)
        {
            if ($ArgList{VPU_SUPPORT} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        ...
        {
            if ($ArgList{TARGET_COPY_OUT_ODM} ne "odm")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "radio")
        {
            if ($ArgList{MTK_A1_FEATURE} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
        ...
        {
            if ($ArgList{DTBO_FEATURE} ne "yes" || $ArgList{DTBO_UPGRADE_FROM_ANDROID_O} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^dtbo(_[ab])?$/i)
        {
            if ($ArgList{DTBO_FEATURE} ne "yes" || $ArgList{DTBO_UPGRADE_FROM_ANDROID_O} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^cache$/i)
        {
            if ($ArgList{MTK_AB_OTA_UPDATER} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^recovery$/i)
        {
            if ($ArgList{TARGET_NO_RECOVERY} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^md1dsp(_[ab])?$/i)
        {
            if ($ArgList{MTK_SINGLE_BIN_MODEM_SUPPORT} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^md1img(_[ab])?$/i)
        {
            if ($ArgList{MTK_SINGLE_BIN_MODEM_SUPPORT} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^vbmeta(_[ab])?$/i)
        {
            if ($ArgList{MTK_BOARD_AVB_ENABLE} ne "true" || $ArgList{MAIN_VBMETA_IN_BOOT} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^vbmeta(_system|_vendor)(_[ab])?$/i)
        {
            if ($ArgList{MTK_BOARD_AVB_ENABLE} ne "true" || $ArgList{DYNAMIC_PARTITION_SUPPORT} ne "yes" || $ArgList{PRODUCT_RETROFIT_DYNAMIC_PARTITIONS} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^(eng_system|eng_vendor)$/i)
        {
            if ($ArgList{MTK_RESERVE_ENG_PARTITIONS} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^product(_[ab])?$/i)
        {
            if ($product_partition_exists eq "N")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^super$/i)
        {
            if ($ArgList{DYNAMIC_PARTITION_SUPPORT} ne "yes" || $ArgList{PRODUCT_RETROFIT_DYNAMIC_PARTITIONS} eq "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        if ($partition_layout_process[$partition_idx]->{Partition_Name} =~ /^md_udc$/i)
        {
            if ($ArgList{MTK_BOARD_USES_METADATA_PARTITION} ne "true")
            {
                splice @partition_layout_process, $partition_idx, 1;
                $partition_idx--;
            }
        }
        my $part_name = GetABPartBaseName($partition_layout_process[$partition_idx]->{Partition_Name});
        my $group = $partition_layout_process[$partition_idx]->{Group};
        if ($group ne "")
        {
            if ($ArgList{DYNAMIC_PARTITION_SUPPORT} eq "yes")
            {
                $dynamic_partition_list{$group}{$part_name} = $partition_layout_process[$partition_idx]->{Size_KB} * 1024;
                if ($ArgList{PRODUCT_RETROFIT_DYNAMIC_PARTITIONS} ne "yes")
                {
                    splice @partition_layout_process, $partition_idx, 1;
                    $partition_idx--;
                }
    ...
    &debug_print_layout("$ENV{OUT_DIR}/4.log", \@partition_layout_process);
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        if ($partition_layout_process[$partition_idx]->{Reserved} eq "N")
        {
            if ($partition_idx != 0 && $partition_layout_process[$partition_idx]->{Region} eq $partition_layout_process[$partition_idx - 1]->{Region})
            {
                ...
                if (exists $AlignPartList{$raw_name})
                {
                    ...
                    #if MTK_PERSIST_PARTITION_SUPPORT=no , do not make persist 8MB aligned for power on write protect to save space
                    if(($partition_layout_process[$partition_idx]->{Partition_Name} eq "protect1" && $ArgList{MTK_SIM_LOCK_POWER_ON_WRITE_PROTECT} ne "yes") ||($partition_layout_process[$partition_idx]->{Partition_Name} eq "persist" && $ArgList{PERSIST_PARTITION_SUPPORT} ne "yes" ))
                    {
        ...
        else
        {
            ...
            {
                $partition_layout_process[$partition_idx]->{Start_Addr_Text} = sprintf("0xFFFF%04x", $partition_layout_process[$partition_idx]->{Start_Addr} / (512 * 1024));
            }
            else
            {
    ...
    &debug_print_layout("$ENV{OUT_DIR}/7.log", \@partition_layout_process);
    ...
    &debug_print_layout("$ENV{OUT_DIR}/8.log", \@partition_layout_process);
...
sub GenYAMLScatFile()
{
    ...
    for ($iter = 0 ; $iter < @partition_layout_cooked ; $iter++)
    {
        ...
        {
            $Scatter_Info{$part->{Partition_Name}}{storage} = "HW_STORAGE_UFS";
        }
        else
        {
...
__TEMPLATE
    ...
    print $ScatterFileFH "${SecondDashes}config_version${colon}V1.1.6\n";
    ...
    if ($ArgList{EMMC_SUPPORT} eq "yes")
    {
        ...
        print $ScatterFileFH "${SecondSpaceSymbol}check_bootloaders_consistency${colon}true\n";
    }
    elsif ($ArgList{UFS_BOOTING} eq "yes")
    {
        print $ScatterFileFH "${SecondSpaceSymbol}storage${colon}UFS\n";
        print $ScatterFileFH "${SecondSpaceSymbol}boot_channel${colon}UFSHCI_0\n";
        printf $ScatterFileFH ("${SecondSpaceSymbol}block_size${colon}0x%x\n", 512 * 1024);
        print $ScatterFileFH "${SecondSpaceSymbol}check_bootloaders_consistency${colon}true\n";
    }
    else
...
sub isDynamicPartition
{
    my $raw_name = shift @_;

    foreach my $group (keys %dynamic_partition_list)
    {
        foreach my $part_name (keys %{$dynamic_partition_list{$group}})
        {
            if ($part_name eq $raw_name)
            {
                return "true";
            }
        }
...
sub GenPartSizeFile_iniFile
{
    ...
    my $part_size_content;
    my $super_part_group_limit = 0;
    my $super_part_group_limit_reserved = 4096*512; #first logical sector is 2048, sector size is 512,  reserve more for future used
    ...
    if ($ArgList{NAND_UBIFS_SUPPORT} eq "yes" && $ArgList{EMMC_SUPPORT} ne "yes" && $ArgList{UFS_BOOTING} ne "yes")
    {
        $IOSIZE           = $ArgList{PAGE_SIZE} * 1024;
        $PEB              = $IOSIZE * 64;
        $LEB              = $IOSIZE * 62;
        $min_ubi_vol_size = $PEB * 28;
        $part_size_content .= sprintf("BOARD_UBIFS_MIN_IO_SIZE:=%d\n",             $IOSIZE);
        $part_size_content .= sprintf("BOARD_FLASH_BLOCK_SIZE:=%d\n",              $PEB);
        $part_size_content .= sprintf("BOARD_UBIFS_VID_HDR_OFFSET:=%d\n",          $IOSIZE);
        $part_size_content .= sprintf("BOARD_UBIFS_LOGICAL_ERASEBLOCK_SIZE:=%d\n", $LEB);
    ...
    foreach my $part (@partition_layout_cooked)
    {
        ...
        {
            if ($part->{Partition_Name} =~ /^(odm)?dtbo(1|_a)?$/i)
            {
                # AOSP build flow using BOARD_DTBOIMG_PARTITION_SIZE instead of BOARD_DTBOIMAGE_PARTITION_SIZE
                $part_size_content .= sprintf("BOARD_DTBOIMG_PARTITION_SIZE:=%d\n",$temp);
            }
            elsif ($part->{Partition_Name} eq "super")
            {
                $part_size_content .= sprintf("BOARD_%s_PARTITION_SIZE:=%d\n",uc($raw_name),$temp);
                if ($ArgList{MTK_AB_OTA_UPDATER} eq "yes")
                {
                    $super_part_group_limit = $temp/2 - $super_part_group_limit_reserved;
                }
                else
                {
                    $super_part_group_limit = $temp - $super_part_group_limit_reserved;
                }
            }
            elsif (&isDynamicPartition($raw_name) eq "true")
            {
                $part_size_content .= sprintf("BOARD_SUPER_PARTITION_%s_DEVICE_SIZE:=%d\n",uc($raw_name),$temp);
            }
            else
            {
                $part_size_content .= sprintf("BOARD_%sIMAGE_PARTITION_SIZE:=%d\n",uc($raw_name),$temp);
            }
        }

        if ($ArgList{NAND_UBIFS_SUPPORT} eq "yes" && $part->{Type} eq "UBIFS" && $ArgList{EMMC_SUPPORT} ne "yes" && $ArgList{UFS_BOOTING} ne "yes")
        {
            ...
            close $inifd;
            $part_size_content .= sprintf("BOARD_UBIFS_%s_MAX_LOGICAL_ERASEBLOCK_COUNT:=%d\n", $part->{Partition_Name}, int((${vol_size} / $LEB) * 1.1));
        }
    }
    if ($ArgList{DYNAMIC_PARTITION_SUPPORT} eq "yes")
    {
        my $super_part_groups = "BOARD_SUPER_PARTITION_GROUPS :=";
        my $super_size = 0;
        foreach my $group (sort keys %dynamic_partition_list)
        {
            $super_part_groups = sprintf("%s %s", $super_part_groups, $group);
            my $group_part_list = sprintf("BOARD_%s_PARTITION_LIST :=", uc($group));
            my $group_size = 0;
            foreach my $part_name (sort keys %{$dynamic_partition_list{$group}})
            {
                $group_part_list = sprintf("%s %s", $group_part_list, $part_name);
                $group_size += $dynamic_partition_list{$group}{$part_name};
            }
            $part_size_content .= sprintf("%s\n", $group_part_list);
            if (scalar keys %dynamic_partition_list == 1 && $group_size > $super_part_group_limit)
            {
                printf("Set group size(%d) to max available size(%d) because group is oversize and only one group is applied\n", $group_size, $super_part_group_limit);
                $group_size = $super_part_group_limit;
            }
            $part_size_content .= sprintf("BOARD_%s_SIZE := %d\n", uc($group), $group_size);
            $super_size = $super_size + $group_size;
        }
        if ($ArgList{PRODUCT_RETROFIT_DYNAMIC_PARTITIONS} eq "yes")
        {
            $part_size_content .= sprintf("BOARD_SUPER_PARTITION_SIZE := %d\n", $super_size);
        }
        $part_size_content .= sprintf("%s\n", $super_part_groups);
    }

    &write_for_rw($part_size, $part_size_content);
}
...
sub GenPGPTFile
{
   ...
   {
       $LBA_size = 512;
       print FH pack("C16896",0x0);#pgpt current max size = (34 - 1[PMBR]) LBA = 33*512 = 16896 bytes for emmc
   }
   elsif ($ArgList{UFS_BOOTING} eq "yes")
   {
       $LBA_size = 4096;
       print FH pack("C20480",0x0);#pgpt current max size = (6-1[PMBR]) LBA =5*4096=20480 bytes for ufs
   }
   ...
   for ($piter = 0 ; $piter < @partition_layout_cooked ; $piter++)
   {
      ...
      {
         seek(FH,0x20,0); #Put sgpt partition size here for lk update usage, new design->32KB for ufs&emmc   old-> 16.5KB for emmc
         print FH pack("Q",$part->{Size_KB}*1024/$LBA_size);
      }
      if($part->{Partition_Name} eq "pgpt")
      {
         seek(FH,0x28,0);
         $FirstUsableLBA = $part->{Size_KB}*1024/$LBA_size;
   ...
   for ($piter = 0 ; $piter < @partition_layout_cooked ; $piter++)
   {
      ...
      {
         print FH pack("Q",$part->{Start_Addr}/$LBA_size);
         print FH pack("Q",($part->{Start_Addr} + $part->{Size_KB}*1024 - 1)/$LBA_size);
      }
      print FH pack("Q",0x0);
...
{
    my $filepath = shift @_;
    my $write_content = shift @_;
    my $read_content = "";
    if (-e $filepath)
    {
        my $READ_HANDLE;
        local $/ = undef;
        open($READ_HANDLE, "<", $filepath) or &error_handler(" Cannot open $filepath for read", __FILE__, __LINE__);
        $read_content = <$READ_HANDLE>;
        close($READ_HANDLE);
    }
    if (($read_content ne $write_content) || ($read_content eq ""))
    {
        ...
        my $WRITE_HANDLE;
        open($WRITE_HANDLE, ">", $filepath) or &error_handler(" Cannot open $filepath for write", __FILE__, __LINE__);
        print $WRITE_HANDLE $write_content;
        close($WRITE_HANDLE);
    }
    push(@GeneratedFile, $filepath);
}
